...
public class NettyHandlerBridge extends SimpleChannelInboundHandler<FullHttpRequest> {
	...
	public void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
		if (!request.decoderResult().isSuccess()) {
			...
			else if (request.protocolVersion().equals(HttpVersion.HTTP_1_0))response.headers().set(HttpHeaderNames.CONNECTION, HttpHeaderValues.KEEP_ALIVE);
			ChannelFuture flushPromise = ctx.writeAndFlush(response);
			if (!keepAlive)flushPromise.addListener(ChannelFutureListener.CLOSE);
			return;
		}
		try {
			if (httpHandler.handle(ctx, isHttps, request))return;
		} catch (Throwable t) {
		...
		if (notFound != null)try (InputStream raf = new FileInputStream(notFound)) {
